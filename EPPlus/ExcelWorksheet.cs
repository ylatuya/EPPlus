            ...
            if (_comments != null)
            {
                ...
                else
                {
                    ...
                    if(_comments.Part==null)
                    {
                        ...
                        var rel = Part.CreateRelationship(PackUriHelperMonoSafe.GetRelativeUri(WorksheetUri, _comments.Uri), TargetMode.Internal, ExcelPackage.schemaRelationships+"/comments",
            ...
            if (_vmlDrawings != null)
            {
                ...
                else
                {
                    ...
                    if (_vmlDrawings.Part == null)
                    {
                        ...
                        var rel = Part.CreateRelationship(PackUriHelperMonoSafe.GetRelativeUri(WorksheetUri, _vmlDrawings.Uri), TargetMode.Internal, ExcelPackage.schemaRelationships + "/vmlDrawing",
                                                         PackagePartForMono.NextRelationshipID);
        ...
        private void SaveTables()
        {
            foreach (var tbl in Tables)
            {
                ...
                if (tbl.Part == null)
                {
                    ...
                    var rel = Part.CreateRelationship(PackUriHelperMonoSafe.GetRelativeUri(WorksheetUri, tbl.TableUri), TargetMode.Internal, ExcelPackage.schemaRelationships + "/table", 
                                                      PackagePartForMono.NextRelationshipID);
                ...
                foreach (ulong cellId in _hyperLinkCells)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                ExcelHyperLink hl = cell.Hyperlink as ExcelHyperLink;
                                sw.Write("<hyperlink ref=\"{0}\" {2}{3}r:id=\"{1}\" />", cell.CellAddress, relationship.Id,                                
                                    string.IsNullOrEmpty(hl.Display) ? "" : "display=\"" + SecurityElement.Escape(hl.Display) + "\" ",
                                    string.IsNullOrEmpty(hl.ToolTip) ? "" : "tooltip=\"" + SecurityElement.Escape(hl.ToolTip) + "\" ");
                            }
                            else
